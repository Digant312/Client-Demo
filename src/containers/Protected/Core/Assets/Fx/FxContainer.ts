...
import FxContainer, {
  IFxContainerProps
} from 'components/Protected/Core/Assets/Fx/FxContainer'
...
interface IMapStateProps {
  assumedAMID: string
  fx: any[] // change equities to fx or futures
  fetching: boolean
  fetchError: string
  darkProfile: boolean
}
...
const mapStateToProps = (state: IState) => ({
  ...
  fx: dataSelector(state).assets.data, // this will be same for fx and futures. Just change the key 'equities' to 'fx', 'futures'
...
const mapDispatchToProps = (dispatch: Function) => ({
  fetchAssets: (userQuery?: any) => {
    const query = {
      assetTypes: 'ForeignExchange', // here change this to 'ForeignExchange' or 'Future'
      ...userQuery
    }
